...
**更新日期：** 2026-01-08  
**当前分支：** copilot/sync-system-and-branch-code
...
```
全品类可回收物预约回收系统
├── recycling.Model (模型层 - 55个类)
├── recycling.DAL (数据访问层 - 20个类)
├── recycling.BLL (业务逻辑层 - 20个类)
...
| **TransportationOrders.cs** | **运输订单实体** | **TransportationOrders** |
| **TransportationOrdrers.cs** | **运输订单（备用）** | **-** |
| **WarehouseReceipts.cs** | **仓库收据实体** | **WarehouseReceipts** |
| **BaseWarehouse.cs** | **基地仓库实体（2026-01-08新增）** | **BaseWarehouse** |

**注意：** TransportationOrdrers.cs 可能是 TransportationOrders.cs 的拼写错误版本，建议核实后移除重复。
...
- **RecyclerListViewModel.cs** - **回收员列表视图**
...
- **BaseWarehouseManagementViewModel.cs** - **基地仓库管理视图模型（2026-01-08新增）**
...
| AdminDAL.cs | Admins | 管理员CRUD、权限管理 |
| SuperAdminDAL.cs | SuperAdmins | 超级管理员CRUD、权限管理 |
...
| StoragePointDAL.cs | StoragePointSummary, StoragePointDetail | 暂存点管理 |
| **TransportationOrderDAL.cs** | **TransportationOrders** | **运输订单管理（新增）** |
| **WarehouseReceiptDAL.cs** | **WarehouseReceipts** | **仓库收据管理（新增）** |
...
| SuperAdminBLL.cs | SuperAdminDAL | 超级管理员权限验证、操作审计 |
...
| StoragePointBLL.cs | StoragePointDAL | 暂存点数据验证和处理 |
| **TransportationOrderBLL.cs** | **TransportationOrderDAL** | **运输订单业务逻辑（新增）** |
| **WarehouseReceiptBLL.cs** | **WarehouseReceiptDAL** | **仓库收据业务逻辑（新增）** |
...
## 4. Common 层（recycling.Common）- 公共工具类

### 工具类（6个）

| 文件名 | 用途 | 主要功能 |
|--------|------|----------|
| Constants.cs | 系统常量定义 | 订单状态、用户角色、Session键名、消息类型等常量 |
| ValidationHelper.cs | 数据验证辅助类 | 手机号、邮箱、用户名、密码强度等验证 |
| StringExtensions.cs | 字符串扩展方法 | 字符串处理、编码、格式化等扩展 |
| DateTimeExtensions.cs | 日期时间扩展方法 | 日期格式化、时间计算等扩展 |
| EmailService.cs | 邮件服务 | 邮件发送功能 |
| LogHelper.cs | 日志辅助类 | 日志记录、文件管理 |
...
- **SessionKeys**: Session键名统一管理
- **SenderType**: 消息发送者类型

#### ValidationHelper.cs - 数据验证
- **手机号验证**: 中国大陆手机号格式（1开头，11位数字）
- **邮箱验证**: 标准邮箱格式验证
- **用户名验证**: 支持中文、英文、数字、下划线
- **密码强度验证**: 长度、大小写、数字、特殊字符要求
- **数字验证**: 纯数字、整数、小数验证
...
**基地管理视图（4个 - 新增）：**
```
BaseManagement.cshtml          - 基地管理（新增）
BaseTransportationManagement.cshtml - 基地运输管理（新增）
BaseWarehouseManagement.cshtml - 基地仓库管理（2026-01-08新增）
TransportationManagement.cshtml - 运输管理（新增）
```
...
### 2025-12-30：系统架构文档同步更新

**更新目的：** 确保架构文档与实际代码完全同步一致
...
3. ✅ 更新Shared视图数：7 → 9个
4. ✅ 更新Model层总数：47 → 48个
5. ✅ 更新View Model数：25 → 26个
6. ✅ 添加Common层完整文档（6个工具类）
7. ✅ 添加遗漏的视图文档：
   - AccountSelfManagement.cshtml（账号自我管理）
   - SuperAdminAccountManagement.cshtml（超级管理员账号管理）
   - SortingCenterWorkerProfile.cshtml（分拣中心工人资料）
   - SortingCenterWorkerEditProfile.cshtml（编辑分拣中心工人资料）
   - SortingCenterWorkerChangePassword.cshtml（修改分拣中心工人密码）
8. ✅ 添加遗漏的Model：SortingCenterWorkerProfileViewModel.cs
9. ✅ 更新文档日期和分支信息
